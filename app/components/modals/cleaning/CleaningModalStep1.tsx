...
const CleaningModalStep1 = () => {
  ...
  const onChangeQuantity = useCallback(
    (item: Record<string, any>, newQuantity: number) => {
      ...
      // Calculate total price more efficiently
      const totalPrice = state.services.reduce((acc: number, serviceItem: { quantity: number; id: number }) => {
        const quantity = serviceItem.id === item.id ? newQuantity : serviceItem.quantity;
        return acc + quantity * person.price;
      }, 0);
