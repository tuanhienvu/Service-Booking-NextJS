...
interface ServiceDetailsPersonCardProps {
  person: serviceProviderExtended;
}
  ...
  return (
    <div className="service-details-person-card grid grid-cols-1 gap-6 px-4 pt-4 sm:p-4 md:grid-cols-2">
      <div className="relative h-[300px] sm:h-[450px]">
        <div className="w-full h-full bg-gradient-to-br from-primary to-primary/70 flex items-center justify-center rounded-badge">
          <div className="text-white text-6xl font-bold">
            {person.location?.charAt(0) || 'S'}
          </div>
        </div>
      </div>
      <div>
        <h1 className="text-xl font-semibold mb-6 sm:text-2xl">Service Provider</h1>
        <div className="flex items-center justify-between mb-4">
          <div className="badge bg-primary/10 text-primary p-4 text-xs">{person.category.title}</div>
          <div className="flex items-center">
            <span className="text-xs text-gray-600 ml-2">Professional Service</span>
          </div>
        </div>
